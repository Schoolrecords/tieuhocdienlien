...
// └──────────────────────────────────────────────────────────┘
var APP_VERSION='3.2';
// ★★★ URL Web App HSS qua Cloudflare Worker proxy (dùng chung với index.html) ★★★
// 2026-05-13: Proxy để bypass adblocker/firewall chặn script.googleusercontent.com.
var DEFAULT_GAS = 'https://thdienlien-api.xebatcheotrt.workers.dev/';
...
// 2026-05-13 — Phương án 3A: chạy song song 5 HS/batch (Apps Script cho 6 concurrent exec/user, dùng 5 cho an toàn).
//   30 HS × ~5s tuần tự = ~2.5 phút  →  30 HS / 5 song song × ~5s = ~30s.
async function hbExportClassToDrive(){
  ...
  var CONCURRENCY = 5;
  // Ước tính: mỗi batch ~5s, làm tròn lên giây (tối thiểu 5s).
  var estSec = Math.max(5, Math.ceil(list.length / CONCURRENCY) * 5);
  var estText = estSec < 60 ? (estSec + ' giây') : ('~' + Math.ceil(estSec / 60) + ' phút');
  if (!confirm('Xuất Word + PDF của ' + list.length + ' HS lớp ' + lop + ' lên Google Drive?\n\nƯớc tính ' + estText + ' (song song ' + CONCURRENCY + ' HS/lần).\n\nKhông đóng/làm mới tab trong lúc đang chạy.')) return;

  var ok = 0, fail = 0, errors = [];
  var startTs = Date.now();

  async function _exportOne(s){
    var blob = await _genHocBaDocx(s);
  ...
  for (var i = 0; i < list.length; i += CONCURRENCY) {
    ...
    var batchNo = Math.floor(i / CONCURRENCY) + 1;
    var totalBatches = Math.ceil(list.length / CONCURRENCY);
    loader('(Lô ' + batchNo + '/' + totalBatches + ') Đang xuất ' + batch.length + ' HS song song… [' + Math.min(i + batch.length, list.length) + '/' + list.length + ']');
    var results = await Promise.allSettled(batch.map(_exportOne));
    for (var j = 0; j < results.length; j++) {
      ...
      if (res.status === 'fulfilled') {
        ...
        fail++;
        var msg = (res.reason && res.reason.message) || String(res.reason);
        errors.push(s.ten + ': ' + msg);
        console.error('[hbExportClassToDrive]', s.ten, res.reason);
      }
    }
  }
...
window.addEventListener('load',function(){
  ...
  if(gasP&&(gasP.indexOf('script.google.com')>=0||gasP.indexOf('workers.dev')>=0)){localStorage.setItem('gas_url',gasP);DEFAULT_GAS=gasP;GAS=gasP;window.history.replaceState({},'',window.location.href.split('?')[0]);setTimeout(function(){toast('✅ Máy đã cấu hình!','ok');},300);}
