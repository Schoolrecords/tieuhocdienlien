...
// ⚠️ BẢO MẬT: KHÔNG hardcode mã ở đây — repo này công khai trên GitHub.
// Mã thật đặt trong Script Properties: Admin panel → "Đổi mã trường" (hàm _updateAuthTokens),
// hoặc Apps Script Editor → Project Settings → Script Properties.
// Nếu Script Properties trống VÀ 2 hằng dưới trống → backend chạy CHẾ ĐỘ MỞ (nguy hiểm, xem log).
const AUTH_TOKEN_GV    = '';   // Mã GV — sửa điểm/nhận xét (đặt qua Script Properties)
const AUTH_TOKEN_ADMIN = '';   // Mã Admin — Admin panel + KĐCL (đặt qua Script Properties)
// ----------------------------------------------------------------------------
//
...
function setup() {
  ...
  Logger.log('   • ' + _expandMinhChungFromHSS().length + ' dòng (khung tiêu chí + minh chứng) vào "MinhChung"');
  _logDivider();
...
/**
 * Helper: dựng lại tab MinhChung (header + dữ liệu). Dùng chung bởi
 * `_populateSheet()` và `setupMinhChungOnly()`.
 *
 * ⚠️ Hàm này CLEAR toàn bộ tab MinhChung trước khi ghi. Mọi ghi chú thủ công
 * (cột "Số/ngày ban hành" / "Nơi ban hành" / "Ghi chú") sẽ MẤT — backup trước
 * nếu cần.
 */
function _populateMinhChungSheet(ss) {
  const HEADER_BG = '#2d8a6e';
  const HEADER_FG = '#ffffff';
  ...
  s6.clearContents();
  s6.clearFormats();
  // ⭐ MỞ RỘNG: chèn các row minh chứng từ DATA_HSS vào dưới mỗi tiêu chí.
  const fullMC = _expandMinhChungFromHSS();
  // ⭐ ÉP ĐỊNH DẠNG TEXT cho TOÀN BỘ VÙNG trước khi ghi (rộng hơn data để chống leak).
  const mcSafeRows = Math.max(fullMC.length + 20, 500);
  ...
  if (fullMC.length) {
    ...
    const rowsWithLinks = fullMC.map(function(r){
      const copy = r.slice();
  ...
  s6.getRange(fullMC.length + 3, 1, 1, 10).merge();
  s6.getRange(fullMC.length + 3, 1).setValue('💡 Mã MC theo CV 5932/BGDĐT-QLCL. Cột "Mã HSS liên kết" = mã hồ sơ trong tab "Danh muc HSS". Link Drive tự động lấy từ tab "Danh muc HSS" khi chạy setup() hoặc setupMinhChungOnly().')
    .setFontStyle('italic').setFontColor('#6b7a72').setWrap(true);

  return { rows: fullMC.length };
}

/**
 * ⭐ AN TOÀN: Chỉ đổ lại tab MinhChung — KHÔNG đụng 5 tab khác (Danh muc HSS,
 * DSGV, DS HocSinh, Hinh Anh, CauHinh). Dùng khi đã cập nhật khung tiêu chí
 * `DATA_MINHCHUNG` hoặc hàm `_expandMinhChungFromHSS()` và muốn refresh tab
 * MinhChung mà KHÔNG mất giáo viên, học sinh, ảnh đã import.
 *
 * Cách chạy: Apps Script → chọn hàm `setupMinhChungOnly` → ▶ Run.
 *
 * Lưu ý: vẫn xóa MinhChung hiện tại trước khi ghi → ghi chú tay (Số/ngày BH,
 * Nơi BH, Ghi chú) MẤT. Backup nếu cần.
 */
function setupMinhChungOnly() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  if (!ss) throw new Error('Apps Script phải mở từ trong Google Sheet (Tiện ích mở rộng → Apps Script).');

  const result = _populateMinhChungSheet(ss);

  // Reset cache để frontend đọc data mới ngay
  try { CacheService.getScriptCache().remove('allData'); } catch(e) {}

  _logDivider();
  Logger.log('✅ Đã đổ lại tab "' + SHEET_MC + '": ' + result.rows + ' dòng (khung tiêu chí + minh chứng).');
  Logger.log('   • KHÔNG đụng đến: Danh muc HSS, DSGV, DS HocSinh, Hinh Anh, CauHinh');
  Logger.log('📋 Sheet: ' + ss.getName() + ' - ' + ss.getUrl());
  _logDivider();
  return { sheet: SHEET_MC, rows: result.rows, url: ss.getUrl() };
}
...
/**
 * ⭐ Sinh danh sách minh chứng đầy đủ từ DATA_MINHCHUNG (khung tiêu chí) + DATA_HSS.
 *
 * Mỗi hồ sơ trong DATA_HSS có cột Mã KĐCL (CSV như "1.1, 1.10") chỉ định nó là
 * minh chứng cho những tiêu chí nào → sinh 1 row minh chứng dưới MỖI tiêu chí
 * khớp. Mã MC tự sinh theo CV 5932/BGDĐT-QLCL: H{TC}-{tieuchi}-{NN}.
 *   • TC      = số tiêu chuẩn (1-5)
 *   • tieuchi = mã tiêu chí đầy đủ (vd "1.1")
 *   • NN      = STT minh chứng trong tiêu chí (01, 02, ...)
 *
 * Mã HSS liên kết = số hồ sơ (vd "1.1.2") → Link Drive sẽ được _buildHssLinkMap()
 * bơm tự động trong _populateSheet().
 *
 * Bỏ qua các Mã KĐCL đặc biệt (TĐG / ĐGN / ĐBCL) — không thuộc 28 tiêu chí.
 */
function _expandMinhChungFromHSS() {
  // Bước 1: Gom các hồ sơ theo từng tiêu chí.
  const byTC = {};  // "1.1" → [{ tenMC, hssCode }, ...]
  DATA_HSS.forEach(function(r){
    const name = String(r[1] || '').trim();
    const kdcl = String(r[4] || '').trim();
    if (!name || !kdcl) return;
    ...
    if (!m) return;
    const hssCode = m[1];          // vd "1.1.2"
    const hssName = m[2].trim();   // vd "Kế hoạch giáo dục nhà trường"
    const tieuChiList = kdcl.split(',').map(function(s){ return s.trim(); }).filter(Boolean);
    tieuChiList.forEach(function(tc){
      if (!/^\d+\.\d+$/.test(tc)) return; // bỏ qua TĐG/ĐGN/ĐBCL
      if (!byTC[tc]) byTC[tc] = [];
      byTC[tc].push({ tenMC: hssName, hssCode: hssCode });
    });
  });

  // Bước 2: Duyệt khung tiêu chí (DATA_MINHCHUNG), sau mỗi row tiêu chí chèn
  // các minh chứng tương ứng.
  const result = [];
  DATA_MINHCHUNG.forEach(function(row){
    result.push(row);
    // Phát hiện row tiêu chí: cột B (tiêu chuẩn) trống, cột C (tiêu chí) khớp "x.y"
    const tieuChuanCol = String(row[1] || '').trim();
    const tieuChiCol   = String(row[2] || '').trim();
    if (!tieuChuanCol && /^\d+\.\d+$/.test(tieuChiCol)) {
      const evidence = byTC[tieuChiCol] || [];
      const tcStandard = tieuChiCol.split('.')[0]; // "1" từ "1.1"
      const tcKey = 'TC' + tcStandard;             // "TC1" — khớp TC_ORDER ở frontend
      evidence.forEach(function(ev, idx){
        const stt = String(idx + 1).padStart(2, '0');
        const maMC = 'H' + tcStandard + '-' + tieuChiCol + '-' + stt;
        // [STT, Tiêu chuẩn, Tiêu chí, Mã MC, Tên minh chứng, Số/ngày BH, Nơi BH, Mã HSS liên kết, Link Drive, Ghi chú]
        // ⭐ Phải điền TC + tiêu chí vào cột B/C để frontend group đúng (xuất Word,
        //    Admin MC table...). Nếu để trống, backend getMinhChung() trả tc='',
        //    tchi='' → group key '' thay vì TC1-TC5 → bảng rỗng.
        result.push(['', tcKey, tieuChiCol, maMC, ev.tenMC, '', '', ev.hssCode, '', '']);
      });
    }
  });
  return result;
...
function setupDbcl() {
  ...
  const PL_ROWS = [
    ...
    [10, 'QĐ thành lập Tổ ĐBCL (96/QĐ-THDL · 20/9/2025)',       'decision', ''],
    ...
    [16, 'Bản cam kết HT với UBND xã Quảng Châu',               'commit',   '']
  ];
