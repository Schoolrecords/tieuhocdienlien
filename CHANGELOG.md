...
## [Lưu Drive lớp — song song 5 HS] — 2026-05-13

### 🎯 Mục tiêu
Tăng tốc nút **"☁️ Lưu Drive lớp"** từ tuần tự (~5s/HS, 30 HS ~2.5 phút) sang song song 5 HS/lần (~30s cho 30 HS, nhanh ~5×).

### ✨ Thay đổi
- **`qlcl-app.js`** (hàm `hbExportClassToDrive`):
  - Thay vòng `for` tuần tự bằng vòng theo batch `Promise.allSettled`, mỗi batch 5 HS (Apps Script cho phép 6 concurrent exec/user, chừa 1 cho an toàn).
  - Loader hiển thị "(Lô N/M) Đang xuất K HS song song… [done/total]".
  - Cập nhật ước tính trong confirm: hiển thị giây nếu <60s, phút nếu ≥60s.
- **`qlcl.html`**: Cập nhật tooltip nút "☁️ Lưu Drive lớp".
- **`index.html`**: Cập nhật SVG + text mô tả trong Modal Hướng dẫn.

### Ghi chú
- KHÔNG đụng server. Mỗi HS vẫn gọi action `exportHocBaSingle` như cũ — chỉ thay đổi cách client điều phối.
- Nếu Apps Script throttle (rare), HS lỗi sẽ rơi vào `errors[]` và thầy chạy lại riêng các HS đó như cũ.
